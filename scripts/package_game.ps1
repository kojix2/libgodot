...
$headlessGodotFlags = @("--headless")
if (-not $onWindows -or $env:CI -or $env:LIBGL_ALWAYS_SOFTWARE) {
    $headlessGodotFlags += @("--rendering-driver", "opengl3", "--audio-driver", "Dummy")
}
...
if ($needsCompile) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "[PackageGame] Failed to compile game.$soExt (exit code: $LASTEXITCODE)"
        exit $LASTEXITCODE
    }
}
...
if (Test-Path $addonBin) {
    if (Test-Path $gameLib) {
        Safe-Copy $gameLib (Join-Path $addonBin "game.$soExt")
    }
    $bridgeLocal = Join-Path $binDir "crystal_bridge.$soExt"
    if (Test-Path $bridgeLocal) {
        Safe-Copy $bridgeLocal (Join-Path $addonBin "crystal_bridge.$soExt")
    }
}
    ...
    foreach ($addonDir in Get-ChildItem -Path $projAddons -Directory) {
        ...
        if (Test-Path $addonBinSrc) {
            ...
            Get-ChildItem -Path $addonBinSrc -Filter $libFilter -File -ErrorAction SilentlyContinue | ForEach-Object {
                Safe-Copy $_.FullName (Join-Path $addonBinDest $_.Name)
            }
        }
    }
}
...
Get-ChildItem -Path $binDir -Filter "*.gdextension*" -File -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
